...
**Review Date:** 2026-03-14 (same day)  
**Reviewer:** Principal-Level Developer (20 years experience)  
**Amendment Author:** Bug Hunter Agent (re-verification pass)

---

### Confirmed Defects (No Change Required)

The following defects were independently confirmed by the principal reviewer and remain at their original severity:

| ID | Principal Status | Severity |
|----|-----------------|----------|
| DEF-001 | ✅ CONFIRMED CRITICAL | CRITICAL — Architecture Fitness Tests Scan Empty Directory |
| DEF-002 | ✅ CONFIRMED CRITICAL | CRITICAL — `load_event_config` Crashes on Malformed YAML |
| DEF-007 | ✅ CONFIRMED HIGH | HIGH — `_owned_client` Not Cleared After `start()` Failure |
| DEF-004 | ✅ CONFIRMED HIGH | HIGH — `_load_error_config_once` Missing F-036 Context Extraction |
| DEF-006 | ✅ CONFIRMED HIGH | HIGH — `StreamingAccumulator.add()` Processes Events After Completion |

---

### Amended Defects

#### DEF-003 — Deny Hook Silent Degradation ⚡ (Severity Maintained, Characterization Updated)

**Original severity:** CRITICAL — Silent Sovereignty Violation  
**Amended characterization:** HIGH — Design Tradeoff with Contract Violation

**Principal's assessment:** The `hasattr` check is reasonable for SDK version compatibility but silent degradation violates the 'no exceptions' principle.

**Amendment:** The principal is correct that `hasattr` is a reasonable SDK version compat pattern. However, the **contract position is unambiguous**. `deny-destroy:DenyHook:MUST:1` states "MUST install on every session" and "MUST NOT have any code path that skips hook installation." There is no "if available" clause, and deny-destroy.md explicitly states: *"Deny + Destroy is NEVER configurable. This is mechanism, not policy."*

The `hasattr` silent fallback is a **contract violation regardless of intent**. The question is not whether the pattern is reasonable in general — it is whether it is acceptable here. It is not.

**Updated fix priority (minimum to best):**
1. **Minimum:** Log `WARNING` when hook method is absent — `logger.warning("[CLIENT] SOVEREIGNTY RISK: register_pre_tool_use_hook not available on session")`
2. **Better (contract-compliant):** Raise `ProviderUnavailableError` — hook installation is non-negotiable per contract

**Severity downgraded from CRITICAL to HIGH** — not because the risk is lower, but because the principal's point that this is a design tradeoff (not an outright coding error) is well-taken. The code was written with intent; the intent is wrong per contract, but the fix is incremental, not emergency.

---

#### DEF-005 — Infinite Recursion in `extract_response_content` ⚡ (Severity Downgraded)

**Original severity:** HIGH — Stack overflow on circular/self-referential objects  
**Amended severity:** LOW — Theoretical risk, low practical impact

**Principal's assessment:** Real SDK returns `Data` objects which DON'T have chained `.data`. Tests use `MockData` with defined structure. MagicMock without spec would cause infinite recursion, but this is a test quality issue, not a production bug.

**Amendment:** Accepted. After re-examination, the recursion terminates in all known real-world paths because actual SDK `Data` objects do not have a `.data` attribute. The risk is limited to:
1. Tests using `MagicMock()` without spec as the `.data` attribute (already noted as a test quality issue under DEF-015/DEF-016)
2. A future SDK refactor that adds chained `.data` attributes (speculative)

**Updated characterization:** The fix (add depth counter) is **optional and conservative**. The real action item is test quality: `MagicMock()` objects used in `data` attribute positions should use `spec=MockData` to prevent silent infinite expansion.

---

#### DEF-008 — Multi-Turn Context Flattening ⚡ (Reclassified as Design Constraint)

**Original severity:** HIGH — Silent Data Loss  
**Amended status:** DESIGN CONSTRAINT — Architecturally correct per `deny-destroy:Ephemeral`

**Principal's assessment:** This is architecturally correct per `deny-destroy:Ephemeral:MUST:1` and `MUST:3`. The orchestrator owns conversation state; provider translates single requests.

**Amendment:** Accepted with clarification. The principal's reading is correct. The Copilot SDK's `create_session()` takes a single prompt string — multi-turn conversation state management is the orchestrator's responsibility, not the provider's. The provider's job per `provider-protocol.md` is "translation — converting between the Amplifier protocol and the Copilot SDK." The `deny-destroy:Ephemeral` clauses make this explicit: no state accumulation, no session reuse.

What looks like "silent data loss" is actually the correct translation: the provider flattens the ChatRequest into a single prompt because the SDK is single-turn by design. A multi-turn ChatRequest reaching this provider indicates the **orchestrator** is the correct place to manage context — and it should pass only the relevant context in the `content` string, not rely on the provider to reconstruct conversation history from structured message objects.

**DEF-008 is NOT a bug.** Removing from bug list, reclassified as a **documentation gap**: the provider's expected input format (single-prompt ChatRequest) should be documented so orchestrator integrators know not to pass rich multi-turn message objects expecting the provider to reconstruct context.

---

### Retracted Defect

#### DEF-010 — `_matches_mapping` Substring Match — PARTIAL RETRACTION WITH DISAGREEMENT

**Principal's verdict:** FALSE POSITIVE — The `"> max="` pattern is meant to match literal circuit breaker log text; `in` substring matching works correctly; `>` is not a regex metacharacter concern.

**Bug Hunter's response:** The principal appears to have addressed a narrower interpretation of DEF-010 than was stated. Re-reading the original DEF-010 text:

> "Pattern matching is substring-based, not exact. Pattern `"Error"` would match every Python exception. Pattern `"TimeoutError"` would match `"LLMTimeoutError"` (Python stdlib)."

The DEF-010 claim was never about `>` being a regex metacharacter. The claim was that **`sdk_patterns` uses substring matching against exception type names**, which is too broad. This remains factually correct: `if pattern in exc_type_name` is a substring check, not exact match.

However, examining `errors.yaml` more carefully, the actual `sdk_patterns` in use are specific class names like `"CircuitBreakerError"`, `"AuthenticationError"`, `"RateLimitError"` — all unique enough that false substring matches are unlikely in practice. The `"TimeoutError"` pattern matching `"LLMTimeoutError"` is a real theoretical risk but requires a future SDK or stdlib to introduce an exception with a colliding substring.

**Amended verdict:** DEF-010 is **not a false positive** but is **overstated at MEDIUM severity**. The risk is real but low. **Downgrading to LOW severity.** The `> max=` regex concern the principal addressed was not in the original DEF-010 text — the principal may have been given a different summary. The core claim (substring matching in `sdk_patterns` could produce false matches) stands.

---

### Root Cause Analysis — Principal's Pattern Identification

The principal identified two systemic patterns underlying the confirmed defects:

#### Pattern 1: F-044/F-045 — Duplicate Logic Divergence

Multiple confirmed defects trace to code that was duplicated and then diverged over time:

| Instance | Duplicate A | Duplicate B | Divergence |
|----------|------------|-------------|------------|
| Error Config | `load_error_config()` in `error_translation.py` | `_load_error_config_once()` in `client.py` | F-036 context extraction added to A, not B (→ **DEF-004**) |
| Defensive Coding | `load_error_config()` uses `.get()` throughout | `load_event_config()` in `streaming.py` uses direct dict access | Defensive pattern not applied consistently (→ **DEF-002**) |
| Path Constant | `pyproject.toml` packages path | `src/` prefix in test paths | Actual path vs test assumption diverged (→ **DEF-001**) |

**Systemic fix:** Eliminate duplicate parsing code. `load_error_config()` should handle both file paths and importlib.resources, making `_load_error_config_once()` unnecessary. `load_event_config()` should adopt the same defensive `.get()` pattern as `load_error_config()`.

#### Pattern 2: Silent Degradation

Three confirmed defects share the root cause of silent no-op fallback where explicit failure is required:

| Defect | Silent Degradation | Contract Violated |
|--------|-------------------|------------------|
| DEF-003 | `hasattr` → no-op if hook method absent | `deny-destroy:DenyHook:MUST:1` |
| DEF-006 | Events accepted after `is_complete = True` | `streaming-contract.md` accumulation invariant |
| DEF-007 | `_owned_client` zombie reference after failed `start()` | `behaviors.md` error resilience |

**Systemic fix:** Add guard clauses at state boundaries. Every setter of `is_complete`, every `start()` call, and every hook installation must either succeed explicitly or raise — never silently no-op.

---

### Updated Priority Order

After principal review and amendments:

| Priority | Defect | Reason |
|----------|--------|--------|
| 1 | **DEF-001** | Two-line fix restores critical architecture guards (fitness tests currently vacuous) |
| 2 | **DEF-002** | Startup crash risk; `.get()` fix is mechanical and low-risk |
| 3 | **DEF-007** | One-line `_owned_client = None` in except block; prevents zombie client state |
| 4 | **DEF-006** | Add `if self.is_complete: return` guard at top of `add()` |
| 5 | **DEF-003** | Upgrade silent no-op to WARNING log (min) or `ProviderUnavailableError` (best) |
| 6 | **DEF-004** | Refactor `_load_error_config_once()` to eliminate duplicate YAML parsing |
| — | DEF-008 | **Removed from bug list.** Design constraint; document expected input format instead. |
| — | DEF-005 | **Downgraded to LOW.** Theoretical risk; add test quality fix (MockData spec). |
| — | DEF-010 | **Downgraded to LOW.** Substring risk in sdk_patterns is real but low in practice. |

---

### Final Defect Status Table (Post-Principal Review)

| ID | Severity | Status | Notes |
|----|----------|--------|-------|
| DEF-001 | CRITICAL | ✅ CONFIRMED | Fix immediately |
| DEF-002 | CRITICAL | ✅ CONFIRMED | Fix immediately |
| DEF-003 | **HIGH** (was CRITICAL) | ✅ CONFIRMED — amended | Design tradeoff but contract violation; needs WARNING or raise |
| DEF-004 | HIGH | ✅ CONFIRMED | F-044/F-045 pattern instance |
| DEF-005 | **LOW** (was HIGH) | ✅ CONFIRMED — amended | Theoretical only; real SDK paths terminate correctly |
| DEF-006 | HIGH | ✅ CONFIRMED | Silent post-completion accumulation |
| DEF-007 | HIGH | ✅ CONFIRMED | Zombie client reference |
| DEF-008 | **DESIGN CONSTRAINT** (was HIGH) | ✅ RECLASSIFIED | Correct architecture; document expected input format |
| DEF-009 | MEDIUM | ✅ CONFIRMED | Weak contract assertion |
| DEF-010 | **LOW** (was MEDIUM) | ✅ MAINTAINED — partial retraction | `> max=` regex concern not the original claim; sdk_patterns substring risk real but low |
| DEF-011 | MEDIUM | ✅ CONFIRMED | Priority ordering gap |
| DEF-012 | MEDIUM | ✅ CONFIRMED | Missing test for mount() failure |
| DEF-013 | MEDIUM | ✅ CONFIRMED | Relative path fragility |
| DEF-014 | MEDIUM | ✅ CONFIRMED AS RISK | Constructor signature risk |
| DEF-015 | LOW | ✅ CONFIRMED | MagicMock without spec |
| DEF-016 | LOW | ✅ CONFIRMED | Incomplete concurrent test |
| DEF-017 | LOW | ✅ CONFIRMED | Post-close semantic inconsistency |
| DEF-018 | LOW | ✅ CONFIRMED | Redundant import inside function |

**Net change from principal review:** 17 actionable defects (DEF-008 reclassified as documentation gap), 2 downgraded in severity (DEF-003, DEF-005), 1 partially maintained against principal's false-positive verdict (DEF-010).

---

*Principal review amendments recorded by Bug Hunter Agent on 2026-03-14.*  
*All disagreements include specific evidence from contract text and/or code.*
