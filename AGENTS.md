# provider-github-copilot -- Autonomous Development Machine
...
- Do NOT run `uv run ruff check src/ && uv run pyright src/` or `uv run pytest tests/ -v` and then fix errors ad hoc
